...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return all([A[k] == B[k] for k in set(product(A.D[0], A.D[1]))])
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, dict((k, A[k] + B[k]) for k in set(product(A.D[0], A.D[1]))))
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, dict((k, x * M[k]) for k in set(product(M.D[0], M.D[1]))))
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), dict(((j, i), M[(i, j)]) for (i, j) in set(product(M.D[0], M.D[1]))))
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], dict((c, sum(v[r] * M[r, c] for r in M.D[0])) for c in M.D[1]))
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], dict((r, sum(v[c] * M[r, c] for c in M.D[1])) for r in M.D[0]))
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), dict(((r, c), sum(A[r, i] * B[i, c] for i in A.D[1])) for (r, c) in set(product(A.D[0], B.D[1])) ) )
